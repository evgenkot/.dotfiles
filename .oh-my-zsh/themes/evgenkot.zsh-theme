...
if [[ -n "$SSH_CONNECTION" ]]; then
	...
	ZSH_ESSEMBEH_PREFIX="%{$fg[magenta]%}[$(echo $SSH_CONNECTION | awk '{print $1}')]%{$reset_color%} "
	# use yellow color to highlight a remote connection
	ZSH_ESSEMBEH_COLOR="yellow"
elif [[ -r /etc/debian_chroot ]]; then 
	# prefix prompt in case of chroot
	ZSH_ESSEMBEH_PREFIX="%{$fg[magenta]%}[chroot:$(cat /etc/debian_chroot)]%{$reset_color%} "
elif [[ -r /.dockerenv ]]; then
	# also prefix prompt inside a docker container
	ZSH_ESSEMBEH_PREFIX="%{$fg[magenta]%}[docker]%{$reset_color%} "
fi
if [[ $UID = 0 ]]; then
	# always use red for root sessions, even in ssh
	ZSH_ESSEMBEH_COLOR="red"
fi
PROMPT='${ZSH_ESSEMBEH_PREFIX}%{$fg[$ZSH_ESSEMBEH_COLOR]%}%n@%M%{$reset_color%}:%{%B$fg[magenta]%}%~%{$reset_color%b%} $(zsh_evgenkot_gitstatus)%(!.#.$) '
